...
let me=null, room=null, players=[], events=[], lastEventId=0, callIndex=0, answered=false, answerType=null, answerAt=0, startedAt=0;
...
async function createRoom(){
  const name=$('createName').value.trim()||'لاعب'; const role=$('createRole').value;
  const roomCode=code();
...
async function joinRoom(){
  ...
  if(error||!r){msg('الغرفة غير موجودة.');return;}
  if(r.status!=='waiting'){msg('الشيفت بدأ بالفعل.');return;}
  const {data:existing}=await db.from('cc_players').select('*').eq('room_id',r.id);
  if((existing||[]).some(p=>p.role===role)){msg('الدور ده محجوز بالفعل في الغرفة. اختار دورًا آخر.');return;}
  const {data:p,error:pe}=await db.from('cc_players').insert({room_id:r.id,name,role}).select().single();
  if(pe){msg('تعذر دخول الغرفة.');console.error(pe);return;}
  me=p; room=r; await enterRoom();
}

async function enterRoom(){
  $('lobby').classList.add('hidden'); $('waiting').classList.remove('hidden');
  $('roomCodeDisplay').textContent=room.code; await refresh();
  waitMsg('في انتظار Agent + Quality + Team Leader.');
}
...
function renderWaiting(){
  ...
  const ready=new Set(players.map(p=>p.role));
  $('startRoomBtn').disabled=!(players.length>=3&&ready.has('agent')&&ready.has('quality')&&ready.has('leader'));
  if(me?.role!=='agent'&&me?.role!=='leader'&&me?.role!=='quality')return;
}

async function startRoom(){
  const ready=new Set(players.map(p=>p.role));
  if(players.length<3||!ready.has('agent')||!ready.has('quality')||!ready.has('leader')){waitMsg('لازم 3 لاعبين: Agent + Quality + Team Leader.');return;}
...
function applyEvent(e){
  if(e.type==='answer'){
    callIndex=e.payload.callIndex; answerType=e.payload.type; answered=true; score=e.payload.score; csat=e.payload.csat; ahtTotal=e.payload.ahtTotal; showAnswered();
  }
  if(e.type==='quality'){qualityScore=e.payload.qualityScore; showQualityResult(e.payload.vote);}
  if(e.type==='leader'){leaderScore=e.payload.leaderScore; showLeaderResult(e.payload.action);}
  if(e.type==='next'){callIndex=e.payload.callIndex; answered=false; answerType=null; renderCall();}
}

function renderGame(){
  $('waiting').classList.add('hidden'); $('game').classList.remove('hidden');
  $('currentRole').textContent=`${roles[me.role].icon} ${me.name} — ${roles[me.role].label}`;
  renderPlayers(); renderCall();
}
...
function renderCall(){
  ...
  const c=calls[callIndex%calls.length]; const agent=players.find(p=>p.role==='agent');
  $('turnInfo').textContent=me.role==='agent'?`🎧 أنت Agent — اختار أفضل رد على العميل`:`👀 أنت ${roles[me.role].label} — تابع المكالمة وانتظر دورك`;
  $('caller').textContent=`📞 العميل: ${c.name}`; $('caller').classList.remove('hidden'); $('scenario').innerHTML=`<h2>مشكلة العميل</h2><p>"${c.issue}"</p>`;
  $('choices').innerHTML=''; $('qualityPanel').classList.add('hidden'); $('leaderPanel').classList.add('hidden'); $('feedback').classList.add('hidden');
...
async function answer(type){
  ...
  const duration=Math.max(1,Math.round((Date.now()-answerAt)/1000))||1; ahtTotal+=duration; score=Math.max(0,score+(type==='good'?2:-12)); csat=Math.max(0,csat+(type==='good'?0:-15));
...
function showAnswered(){
  ...
  if(me.role==='quality')showQualityPanel(); if(me.role==='leader')showLeaderPanel();
  $('quality').textContent=`${Math.round((score+qualityScore)/2)}%`; $('csat').textContent=`${csat}%`;
}
...
async function leaderVote(action){if(me.role!=='leader')return;leaderScore=Math.max(0,leaderScore+(action==='coach'?2:-3));await db.from('cc_events').insert({room_id:room.id,player_id:me.id,type:'leader',payload:{action,leaderScore}});await db.from('cc_rooms').update({status:'playing'}).eq('id',room.id);await refresh();}
...
$('createRoomBtn').onclick=createRoom; $('joinRoomBtn').onclick=joinRoom; $('startRoomBtn').onclick=startRoom;
setInterval(refresh,1500);

// The first call timer starts when the room enters the game.
const originalRenderGame=renderGame;
renderGame=function(){originalRenderGame();if(!answerAt)answerAt=Date.now();};
